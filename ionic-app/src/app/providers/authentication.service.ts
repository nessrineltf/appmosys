...
export class AuthenticationService {
    ...
    getprofile() {
        ...
        return this.http.get('http://localhost:8080/api/admin/users', { headers});
    }
    getUser() {

        const headers = new HttpHeaders({ authorization: 'Bearer ' + this.jwt});

        return this.http.get('http://localhost:8080/api/auth/info', { headers});
    }
    isPatient() {
        return this.roles.indexOf('Patient') >= 0 ;
      }
}
